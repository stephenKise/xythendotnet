...
page_header("Bank of Sylisten");
output("`c`b`^B`b`i`ma`Mn`tk`i `)o`7f `b`LS`b`i`ly`kl`3i`#s`3t`ke`ln`i`c`n`n");
$op = httpget('op');
if ($op==""){
  checkday();
  ...
  output("`@Jezabelle`6, a petite woman in an immaculately tailored business dress, greets you from behind reading spectacles with polished silver frames.`n`n");
  output("`6\"`5Greetings, my good lady,`6\" you greet her, \"`5Might I inquire as to my balance this fine day?`6\"`n`n");
  output("`@Jezabelle`6 blinks for a moment and then smiles, \"`@Hmm, `&%s`@, let's see.....`6\" she mutters as she scans down a page in her ledger.",$session['user']['name']);
	...
	if ($session['user']['goldinbank']>=0){
		output("`@Jezabelle`6 tells you, \"`@Just so that you are fully aware of our policies, you may only transfer `^%s`@ gold per the recipient's level.",getsetting("transferperlevel",25));
		...
		output("`@Jezabelle`6 tells you that she refuses to transfer money for someone who is in debt.");
	}
}elseif($op=="transfer2"){
	...
	if (db_num_rows($result)==1){
		...
		output("`@Jezabelle`6 looks at you disdainfully and coldly, but politely, suggests you try narrowing down the field of who you want to send money to just a little bit!`n`n");
		...
		output("`@Jezabelle`6 blinks at you from behind her spectacles, \"`@I'm sorry, but I can find no one matching that name who does business with our bank!  Please try again.`6\"");
	}
}elseif($op=="transfer3"){
	$amt = abs((int)httppost('amount'));
	$to = httppost('to');
	...
	if ($session['user']['gold']+$session['user']['goldinbank']<$amt){
		output("`@Jezabelle`6 stands up to her full, but still diminutive height and glares at you, \"`@How can you transfer `^%s`@ gold when you only possess `^%s`@?`6\"",$amt,$session['user']['gold']+$session['user']['goldinbank']);
	}else{
		$sql = "SELECT name,acctid,level,transferredtoday FROM " . db_prefix("accounts") . " WHERE login='$to'";
		$result = db_query($sql);
		if (db_num_rows($result)==1){
			$row = db_fetch_assoc($result);
			$maxout = $session['user']['level']*getsetting("maxtransferout",105);
			$maxtfer = $row['level']*getsetting("transferperlevel",125);
			if ($session['user']['amountouttoday']+$amt > $maxout) {
				output("`@Jezabelle`6 shakes her head, \"`@I'm sorry, but I cannot complete that transfer; you are not allowed to transfer more than `^%s`@ gold total per day.`6\"",$maxout);
			}else if ($maxtfer<$amt){
				output("`@Jezabelle`6 shakes her head, \"`@I'm sorry, but I cannot complete that transfer; `&%s`@ may only receive up to `^%s`@ gold per day.`6\"",$row['name'],$maxtfer);
			}else if($row['transferredtoday']>=getsetting("transferreceive",3)){
				output("`@Jezabelle`6 shakes her head, \"`@I'm sorry, but I cannot complete that transfer; `&%s`@ has received too many transfers today, you will have to wait until tomorrow.`6\"",$row['name']);
			}else if($amt<(int)$session['user']['level']){
				output("`@Jezabelle`6 shakes her head, \"`@I'm sorry, but I cannot complete that transfer; you might want to send a worthwhile transfer, at least as much as your level.`6\"");
			}else if($row['acctid']==$session['user']['acctid']){
				output("`@Jezabelle`6 glares at you, her eyes flashing dangerously, \"`@You may not transfer money to yourself!  That makes no sense!`6\"");
			}else{
				...
				db_query($sql);
				output("`@Jezabelle`6 smiles, \"`@The transfer has been completed!`6\"");
			...
			output("`@Jezabelle`6 looks up from her ledger with a bit of surprise on her face, \"`@I'm terribly sorry, but I seem to have run into an accounting error, would you please try telling me what you wish to transfer again?`6\"");
		}
	}
}elseif($op=="deposit"){
	output("`0");
	rawoutput("<form action='bank.php?op=depositfinish' method='POST'>");
	$balance = translate_inline("`@Jezabelle`6 says, \"`@You have a balance of `^%s`@ gold in the bank.`6\"`n");
	$debt = translate_inline("`@Jezabelle`6 says, \"`@You have a `\$debt`@ of `^%s`@ gold to the bank.`6\"`n");
  ...
  addnav("","bank.php?op=depositfinish");
	...
	$notenough = translate_inline("`\$ERROR: Not enough gold in hand to deposit.`n`n`^You plunk your `&%s`^ gold on the counter and declare that you would like to deposit all `&%s`^ gold of it.`n`n`@Jezabelle`6 stares blandly at you for a few seconds until you become self conscious and recount your money, realizing your mistake.");
	$depositdebt = translate_inline("`@Jezabelle`6 records your deposit of `^%s `6gold in her ledger. \"`@Thank you, `&%s`@.  You now have a debt of `\$%s`@ gold to the bank and `^%s`@ gold in hand.`6\"");
	$depositbalance= translate_inline("`@Jezabelle`6 records your deposit of `^%s `6gold in her ledger. \"`@Thank you, `&%s`@.  You now have a balance of `^%s`@ gold in the bank and `^%s`@ gold in hand.`6\"");
	...
	$balance = translate_inline("`@Jezabelle`6 scans through her ledger, \"`@You have a balance of `^%s`@ gold in the bank.`6\"`n");
	$debt = translate_inline("`@Jezabelle`6 scans through her ledger, \"`@You have a `\$debt`@ of `^%s`@ gold to the bank.`6\"`n");
	...
	$balance = translate_inline("`@Jezabelle`6 scans through her ledger, \"`@You have a balance of `^%s`@ gold in the bank.`6\"`n");
	$debt = translate_inline("`@Jezabelle`6 scans through her ledger, \"`@You have a `\$debt`@ of `^%s`@ gold in the bank.`6\"`n");
	rawoutput("<form action='bank.php?op=withdrawfinish' method='POST'>");
	output_notl($session['user']['goldinbank']>=0?$balance:$debt,abs($session['user']['goldinbank']));
	...
	if ($amount>$session['user']['goldinbank'] && httppost('borrow')=="") {
		...
		output("`@Jezabelle`6 looks at you for a few moments without blinking, then advises you to take basic arithmetic.  You realize your folly and think you should try again.");
		...
		if ($lefttoborrow<=$session['user']['goldinbank']+$maxborrow){
			...
			if ($lefttoborrow-$session['user']['goldinbank'] > $maxborrow){
				...
				output("`@Jezabelle`6 looks up your account and informs you that you may only borrow up to `^%s`6 gold.", $maxborrow);
			}else{
				if ($didwithdraw) {
				...
				output("`@Jezabelle`6 records your withdrawal of `^%s `6gold in her ledger. \"`@Thank you, `&%s`@.  You now have a debt of `\$%s`@ gold to the bank and `^%s`@ gold in hand.`6\"", $amount,$session['user']['name'], abs($session['user']['goldinbank']),$session['user']['gold']);
			}
		}else{
			output("`6Considering the `^%s`6 gold in your account, you ask to borrow `^%s`6. `@Jezabelle`6 peers through her ledger, runs a few calculations and then informs you that, at your level, you may only borrow up to a total of `^%s`6 gold.", $session['user']['goldinbank'], $lefttoborrow-$session['user']['goldinbank'], $maxborrow);
		}
	}else{
		$session['user']['goldinbank']-=$amount;
		$session['user']['gold']+=$amount;
		debuglog("withdrew $amount gold from the bank");
		output("`@Jezabelle`6 records your withdrawal of `^%s `6gold in her ledger. \"`@Thank you, `&%s`@.  You now have a balance of `^%s`@ gold in the bank and `^%s`@ gold in hand.`6\"", $amount,$session['user']['name'], abs($session['user']['goldinbank']),$session['user']['gold']);
	}
}
villagenav();
addnav("Money");
