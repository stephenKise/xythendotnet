				...
				if ($badguy['dead'] == true || $badguy['creaturehealth'] <= 0){
					...
					if (getsetting("instantexp",false) == true && $session['user']['alive'] && $options['type'] != "pvp" && $options['type'] != "train") {
						if (!isset($badguy['expgained']) || $badguy['expgained'] == false) {
							...
							$roundedexp = round($badguy['creatureexp']/count($newenemies));
							$clanexp = floor($roundedexp*0.1);
							output("`#You receive `^%s`# total experience!`n`0",$roundedexp);
							$session['user']['experience']+=$roundedexp;
							if ($session['user']['clanid'] != 0){
								db_query("UPDATE ".db_prefix('clans')." SET clanexp = clanexp + ".$clanexp." WHERE clanid = ".$session['user']['clanid']);
								output("`#Your dimension receives `^%s `#experience!`n`0",$clanexp);
							}
...
function battle_player_attacks() {
	...
	if ($creaturedmg==0){
		...
		$creaturedmg = floor($creaturedmg/4);
