...
/*if (isset($_GET['test'])){
	output("<fieldset class=\"field_set\"><legend><b>Character Creation:</b></legend>",true);
	rawoutput("<form autocomplete=\"off\" action=\"create.php?op=create$refer\" method='POST' onSubmit=\"md5pass();\">");
	rawoutput("<input class=\"input_create\" type='hidden' name='passlen' id='passlen' value='0'>");
	output("<table>",true);
	output("<tr><td>Username:</td><td><input class=\"input_create\" name=\"name\" type=\"text\" required=\"required\"></td></tr>",true);
	output("<tr><td>Password:</td><td><input class=\"input_create\" name=\"password\" type=\"password\" required=\"password\"></td></tr>",true);
	output("<tr><td>Re-type Password:</td><td><input class=\"input_create\" name=\"check_password\" type=\"password\" required=\"password\"></td></tr>",true);
	output("<tr><td>Email Address:</td><td><input class=\"input_create\" name=\"email\" type=\"email\" required=\"email\"></tr>",true);
	output("</table>",true);
	output("<input type=\"radio\" name=\"sex\" value=\"0\" checked>`LMale",true);
	output("<input type=\"radio\" name=\"sex\" value=\"1\">`%Female`n`n",true);
...
//	output("<p>Suggestions: <span id=\"txtHint\"></span></p>",true);
...
if ($op=="val"){
	...
	$sql = "SELECT acctid,login,password,name,lastip,emailaddress FROM ". db_prefix("accounts") . " WHERE emailvalidation='$id' AND emailvalidation!=''";
	$result = db_query($sql);
...
if (getsetting("allowcreation",1)==0){
	...
	if ($op=="create"){
		...
		if (soap($shortname)!=$shortname){
			output("`\$Error`^: Bad language was found in your name, please revise it.`n");
			$op="";
		}else{
			$blockaccount=false;
...
//			if (check_proxy() != "VALID"){
//				$msg.=translate_inline("You may not use a proxy to enter this site.");
//				$blockaccount = true;
//			}
			...
			if (!$blockaccount){
				...
				if (db_num_rows($result)>0){
					...
					if (db_affected_rows($LINK)<=0){
						...
						$sql = "SELECT acctid,login,name,lastip,emailaddress FROM " . db_prefix("accounts") . " WHERE login='$shortname'";
						$result = db_query($sql);
						$row = db_fetch_assoc($result);
						$args = httpallpost();
						...
						db_query($sql_output);
						//end
						$searchip = db_num_rows(db_query("SELECT name FROM accounts WHERE lastip='".$row['lastip']."' AND acctid!='".$row['acctid']."'"));
						$searchemail = db_num_rows(db_query("SELECT name FROM accounts WHERE emailaddress='".$row['emailaddress']."' AND acctid!='".$row['acctid']."'"));
						
						$inform_sql = "SELECT acctid FROM ".db_prefix("accounts")." WHERE (superuser&".SU_EDIT_COMMENTS.")";
		            	$inform_result = db_query($inform_sql);
		            	while($mail_row = db_fetch_assoc($inform_result)){
							require_once("lib/systemmail.php");
			            	systemmail($mail_row['acctid'],"`bCharacter Created`b","".$row['login']."(ACCTID: ".$row['acctid'].", IP: ".$row['lastip'].", Email: ".$row['emailaddress'].") has been created on Xythen.`n`n[<a href=\"mail.php?op=write&to=".rawurlencode($row['login'])."\">Welcome User</a>]`n`nThis IP has `b".$searchip."`b other registered characters.`nThis email has `b".$searchemail."`b other registered characters.");
						}
	...
	if ($op==""){
		...
		//CREATION FORM MADE BY MAVERICK
		output(
			"<fieldset class='characterCreation'>
				<legend>
					Character Creation:
				</legend>",
			true
		);
		rawoutput("<form autocomplete=\"false\" action=\"create.php?op=create$refer\" method='POST' onSubmit=\"md5pass();\">");
		rawoutput("<input type='hidden' name='passlen' id='passlen' value='0'>");
		output("<table>",true);
		output("<tr><td>Username:</td><td><input name=\"name\" type=\"text\" required=\"required\" autocomplete=\"off\"></td></tr>",true);
		output("<tr><td>Password:</td><td><input name=\"pass1\" type=\"password\" required=\"password\" autocomplete=\"off\"></td></tr>",true);
		output("<tr><td>Re-type Password:</td><td><input name=\"pass2\" type=\"password\" required=\"password\"></td></tr>",true);
		output("<tr><td>Email Address:</td><td><input name=\"email\" type=\"email\" required=\"email\"></tr>",true);
		output("</table>",true);
		output("<input type=\"radio\" name=\"sex\" value=\"0\" checked>`LMale",true);
		output("<input type=\"radio\" name=\"sex\" value=\"1\">`%Female`n`n",true);
		rawoutput("<input type='submit' class='button' value='Create'>");
