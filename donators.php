...
if ($op=="add2"){
	...
	$sql="SELECT name,acctid FROM ".db_prefix("accounts")." WHERE acctid=$id;";
	$result=db_query($sql);
	$row=db_fetch_assoc($result);
	output("%s donation points added to %s`0, reason: `^%s`0",$amt,$row['name'],$reason);
	
	$sql2 = db_query("SELECT transfers FROM accounts WHERE acctid='".$row['name']."'");
	$row2 = db_fetch_assoc($sql2);
	$transfers = @unserialize($row2['transfers']);
	if (!is_array($transfers)) $transfers = array();
	array_push($transfers,"".$session['user']['name']."(ACCTID: ".$session['user']['acctid'].") gave ".$amt." DPS to ".$row['name']."(ACCTID: ".$row['acctid'].") for ".$reason."");
	$transfers = array_unique($transfers);
	while (count($transfers) > 100){
		array_shift($transfers);
	}
	db_query("UPDATE accounts SET transfers='".@serialize($transfers)."' WHERE acctid='".$row['acctid']."'");
