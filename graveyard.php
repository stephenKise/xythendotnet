...
$deathoverlord=getsetting('deathoverlord','`$Jester');
...
case "run":
		...
		tlschema("nav");
		addnav("G?Return to the Wasteland","graveyard.php");
		output("`)As you try to flee, you are summoned back to the fight!`n`n");
		$battle=false;
		redirect('graveyard.php');
	break;
case "fight":
		$battle = true;
...
if ($battle){
	...
	if ($victory) {
		...
		if ($defeat){
			...
			addnews("`)%s`) has been defeated even in death!",$session['user']['name'],$badguy['creaturename'],$taunt);
