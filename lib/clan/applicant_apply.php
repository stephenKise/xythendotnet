<?php
		...
		$clanexp = db_fetch_assoc(db_query("SELECT clanexp AS amt FROM clans WHERE clanid = 1"));
		$clanlevel = 1;
		while($clanexp['amt']>=experience($clanlevel)){
			$current = $clanlevel;
			$clanlevel++;
		}

		$clanmembers = db_fetch_assoc(db_query("SELECT COUNT(acctid) AS amt FROM accounts WHERE clanid = 1"));

				if ($clanmembers['amt']	>= $current){
					output("`%%s`7 frowns and informs you that the guild you wish to join needs to level up before being allowed to accept anymore members!",$registrar);
				}else{
					...
					$result = db_query($sql);
					$sql = "DELETE FROM . ".  db_prefix("mail") . " WHERE msgfrom=0 AND seen=0 AND subject='$apply_subj'";
					db_query($sql);
					while ($row = db_fetch_assoc($result)){
						$msg = "`^You have a new guild applicant! `&{$session['user']['name']}`^ has completed a membership application for your Guild!";
					...
					if ( nltoappon($row['clandesc']) != "" ) {
						...
						$mail = "`&Did you remember to read the description of the Guild of your choice before applying?  Note that some Guilds may have requirements that you have to fulfill before you can become a member.  If you are not accepted into the Guild of your choice anytime soon, it may be because you have not fulfilled these requirements.  For your convenience, the description of the Guild you are applying to is reproduced below.`n`n`c`#%s`@ <`^%s`@>`0`c`n%s";
					...
					if (db_num_rows($result)>0){
						output("`7You ask %s`7 for a Guild membership application form.",$registrar);
						output("She opens a drawer in her desk and pulls out a form.  It contains only two lines: Name and Guild Name.");
						...
						output("Noticing your attempt to write on the form with your %s, %s`7 claims the form back from you, writes %s`7 on the first line, and asks you the name of the Guild that you'd like to join:`n`n",$session['user']['weapon'],$registrar,$session['user']['name']);
		...
		/*//*/					if (isset($row['handled']) && $row['handled']) continue;
								...
								output_notl("&#149; <a href='clan.php?op=apply&to=%s'>%s</a> %s`n",
										$row['clanid'],
										full_sanitize(str_replace("amp;","",htmlentities($row['clanname'], ENT_COMPAT, getsetting("charset","ISO-8859-1")))),
										$memb, true);
						...
						output("`7You ask %s`7 for a Guild membership application form.",$registrar);
						output("She stares at you blankly for a few moments, then says, \"`5Sorry pal, no one has had enough gumption to start up a Guild yet.  Maybe that should be you, eh?`7\"");
