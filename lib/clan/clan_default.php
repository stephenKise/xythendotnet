<?php
		...
		if ($claninfo['clanmotd'] != '') {
			...
			output(nltoappon($claninfo['clanmotd'])."`n",true);
			//rawoutput("</div>");
			output_notl("`n");
		}
		...
		if ($claninfo['clandesc'] != '') {
			...
			output_notl($claninfo['clandesc'],true);
			modulehook("}collapse");
		}
		...
		if ($session['user']['clanrank']>CLAN_MEMBER){
			addnav("Change Descriptions","clan.php?op=motd");
		}
		addnav("M?View Members","clan.php?op=membership");
		addnav("Leave Guild","clan.php?op=withdrawconfirm");
?>