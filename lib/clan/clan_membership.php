<?php
		...
		output("This is your current Guild membership:`n");
		...
		$confirm = translate_inline("Are you sure you wish to remove this member from your Guild?");
		rawoutput("<tr class='trhead'><td>$rank</td><td>$name</td><td>$lev</td><td>$dk</td><td>$jd</td><td>$lo</td>".($session['user']['clanrank']>CLAN_MEMBER?"<td>$rops</td><td>$ops</td>":"")."</tr>",true);
		$i=0;
		$tot = 0;
		...
		while ($row=db_fetch_assoc($result)){
			...
			if ($session['user']['clanrank']>CLAN_MEMBER){
				rawoutput("<td>");
				if ($row['login'] != $session['user']['login'] && $row['clanrank'] <= $session['user']['clanrank']){
					rawoutput("<select name='rank' onchange=\"window.location.href = this.value\">");
					for ($i = 0; $i <= 31; $i++){
						if (isset($ranks[$i])){
							$value = "clan.php?op=membership&setrank=$i&who=".rawurlencode($row['login'])."&whoname=".rawurlencode($row['name'])."&whoacctid=".$row['acctid'];
							rawoutput("<option value='$value' ".($i==$row['clanrank']?"selected":"").">");
								output_notl("%s",$ranks[$i]);
								addnav("",$value);
							rawoutput("</option>");
						}
					}
					...
					output_notl("%s",$ranks[$row['clanrank']]);
				}
				...
				if ($row['clanrank']==CLAN_FOUNDER && $row['clanrank']>CLAN_APPLICANT && $row['login']==$session['user']['login']){
					output_notl("<br>[ <a href='clan.php?op=membership&setrank=".clan_previousrank($ranks,$row['clanrank'])."&whoacctid=".$row['acctid']."'>$stepdown</a> ]",true);
		...
		rawoutput("</table>");
		output("`n`n`^This Guild has a total of `\$%s`^ dragon kills.",$tot);
?>
