<?php
		page_header("Description and MoTD");
		addnav("Management");
		global $session;
		if ($session['user']['clanrank']>=CLAN_OFFICER){
			$clanad = substr(httppost('clanad'),0,9999);
			if (httppostisset('clanad') &&
					stripslashes($clanad)!=$claninfo['clanad']){
				$sql = "UPDATE " . db_prefix("clans") . " SET clanad='$clanad',motdauthor={$session['user']['acctid']} WHERE clanid={$claninfo['clanid']}";
				db_query($sql);
				invalidatedatacache("clandata-{$claninfo['clanid']}");
				$claninfo['clanad']=stripslashes($clanad);
				output("Updating Advertisement`n");
				$claninfo['motdauthor']=$session['user']['acctid'];
			}
			$clanmotd = substr(httppost('clanmotd'),0,9999);
			...
			if (httppostisset('clandesc') &&
				...
				$sql = "UPDATE " . db_prefix("clans") . " SET clandesc='".addslashes(substr(stripslashes($clandesc),0,9999))."',descauthor={$session['user']['acctid']} WHERE clanid={$claninfo['clanid']}";
				db_query($sql);
				invalidatedatacache("clandata-{$claninfo['clanid']}");
			...
			output("`&`bCurrent Advertisement:`b `2`n");
			output_notl(nltoappon($claninfo['clanad'])."`n");

			rawoutput("<form action='clan.php?op=motd' method='POST'>");
			addnav("","clan.php?op=motd");
			output("`&`bMoTD:`b `7(9999 chars)`n");
			...
			output("`n`&`bDescription:`b `7(9999 chars)`n");
			$blocked = translate_inline("Your Guild has been blocked from posting a description.`n");
			...
			output("`&`bAdvertisement:`b `7(1000 chars)`n");
			rawoutput("<textarea name='clanad' cols='40' rows='5' class='input' style='width: 66%'>".htmlentities($claninfo['clanad'], ENT_COMPAT, getsetting("charset", "ISO-8859-1"))."</textarea><br>");
			...
			rawoutput("</form>");
			invalidatedatacache("clandata-{$session['user']['clanid']}");
		}else{
			output("You do not have authority to change your Guild's motd or description.");
		}
