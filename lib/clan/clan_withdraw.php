<?php
		...
		if ($session['user']['clanrank']>=CLAN_LEADER){
			...
			if ($row['c']==0){
				...
				if ($row = db_fetch_assoc($result)){
					...
					db_query($sql);
					output("`^As you were the last member of this guild, it has been deleted.");
				}
			}else{
		...
		// $withdraw_subj = array("`\$Guild Withdraw: `&%s`0",$session['user']['name']);
		$withdraw_subj = "`\$Guild Withdraw: `&{$session['user']['name']}`0";
		// $msg = array("`^One of your Guild members has resigned their membership.  `&%s`^ has surrendered their position within your Guild!",$session['user']['name']);
		$msg = "`^One of your Guild members has resigned their membership. `&{$session['user']['name']}`^ has surrendered their position within your Guild!";
		...
		output("`&You have withdrawn from your Guild.");
		addnav("Management");
