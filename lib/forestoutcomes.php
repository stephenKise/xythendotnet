...
function forestvictory($enemies,$denyflawless=false){
	...
	$exp = round((($session['user']['level']*20)*e_rand(5,10)+15)/3);
	$expbonus = 0;
	if ($gold) {
		output("`#You receive `^%s`# gold!`n",$gold);
//		INSERT NEW RACE INCREASED GOLD MODIFICATION
		modulehook("forestgold");
		debuglog("received gold for slaying a monster.",false,false,"forestwin",$badguy['creaturegold']);
	}
	// No gem hunters allowed!
	$args = modulehook("alter-gemchance", array("chance"=>getsetting("forestgemchance", 10)));
	$ranks = array(0 => 0,
			500 => 1,
			1000 => 2,
			5000 => 3,
	...
	foreach ($ranks as $key => $val){
		if ($session['user']['donation'] >= $key) $args['chance']-=$val;
	}
...
// 		debug($session['user']['clanid']);
		...
		{
			$totalexp += 5500;
			increment_module_pref('kills',20,'monsterkills',$session['user']['acctid']);
		}
		...
		debug($totalexp);
		$clanexp = floor($totalexp*0.1);
		output("`#You receive `^%s`# total experience!`n`0",$totalexp);
		$session['user']['experience']+=$totalexp;
// 		debug($session['user']['clanid']);
		if ($session['user']['clanid'] != 0){
			db_query("UPDATE ".db_prefix('clans')." SET clanexp = clanexp + ".$clanexp." WHERE clanid = ".$session['user']['clanid']);
			output("`#Your guild receives `^%s `#experience!`n`0",$clanexp);
		}
	}
...
function forestdefeat($enemies,$where="in the forest"){
	...
	addnews("`%%s`4 has been slain!",$session['user']['name'],$where,$badguy['creaturename'],$taunt);
	...
	$session['user']['gems']=0;
	output("`4All of your experience, gold, and gems on as been lost!`n");
	page_footer();
}
