...
// translator ready
// mail ready
function motd_log_pages($current, $total, $group){
	$t = floor(($current-1)/$group)*$group;
	$u = (($t + $group) >= $total ? $total : ($t + $group));
	$v = array();
	for ($i = 1; $i <= $total; $i++){
		if ($i > $t && $i <= $u) {
			output("<a href='motd.php?op=changes&page=$i'>`#".($current==$i?"`b$i`b":"$i")."`0</a>",true);
		} else {
			$fi = floor($i/$group)*$group;
			if (!in_array($fi, $v) && $fi <> $t){
				$v[] = $fi;
				$st1 = $fi+1;
				$st2 = ($fi+$group)>$total?$total:($fi+$group);
				rawoutput("<a href='motd.php?op=changes&page=$st1'>");
				output($st1.($st1 <> $st2 ? "-".$st2 : ""));
				rawoutput("</a>");
			}
		}
	}
...
function pollitem($id,$subject,$body,$author,$date,$showpoll=true){
	...
	output_notl("`2%s`0`n", stripslashes(nltoappon($body['body'])),TRUE);
...
function motd_del($id) {
	$sql = "SELECT motdtype FROM " . db_prefix("motd") . " WHERE motditem=\"$id\"";
	$res = db_query($sql);
	$row = db_fetch_assoc($res);
	$header = ($row['motdtype']?"?op=polls":"");
	...
	invalidatedatacache("lastmotd");
	invalidatedatacache("motddate");
	header("Location: motd.php".$header);
	exit();
}
?>