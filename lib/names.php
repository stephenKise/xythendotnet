...
function base_name($acctid=false)
{
	global $session;
	if ($acctid == false)
		$acctid = $session['user']['acctid'];

	$sql = db_query("SELECT name, ctitle, title FROM accounts WHERE acctid = '$acctid'");
	$row = db_fetch_assoc($sql);
	$name = $row['name'];
	$title = str_replace("`0","",$row['title']);
	$ctitle = str_replace("`0","",$row['ctitle']);
	$name = str_replace($ctitle, "", $name);
