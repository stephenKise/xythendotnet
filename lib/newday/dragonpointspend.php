<?php
if ($dkills-$dp > 1) {
	page_header("TentroMech Points");
	output("`@You earn one TentroMech point each time you slay the TentroMech.");
	output("Advancements made by spending TentroMech points are permanent!");
	output("`n`nYou have `^%s`@ unspent TentroMech points.", $dkills-$dp);
	output("How do you wish to spend them?`n`n");
	output("Be sure that your allocations add up to your total unspent TentroMech points.");
	...
	foreach ($labels as $type=>$label) {
		...
		output("`@You have `&1`@ unspent TentroMech point. ");
	...
	output("Advancements made by spending TentroMech points are permanent!");
		...
		output("`n`nCurrently, the TentroMech points you have already spent are distributed in the following manner.");
