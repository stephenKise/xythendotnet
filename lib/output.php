...
function appoencode($data,$priv=false){
	global $nestedtags,$session,$colors;
	$start = 0;
	$out="";
	if( ($pos = strpos($data, "`")) !== false) {
		...
		if (!isset($nestedtags['s'])) $nestedtags['s']=false;
		if (!isset($nestedtags['u'])) $nestedtags['u']=false;
...
function getstatcolor($id){
	...
	$stat_color = get_module_pref("user_statcolor","additionalbioinfos",$id);
	$scolor = $allcolors[$stat_color];
	if ($scolor == "") $scolor = "`^";
	else $scolor = "`$scolor";
	return $scolor;
}
...
function outputcolors(){
	...
	foreach($allcolors as $color){
		if ($colors == "") $colors .= "`$color$color ";
		else if ($num == 40) $colors .= "`$color$color`^.";
...
function addnav($text,$link=false,$priv=false,$pop=false,$popsize="500x300"){
	...
	if ($link===false) {
		...
		if ($text==""){
			...
			$args0 = modulehook("addnav",array("text" => $text));
			$text = $args0['text'];
			if (!isset($navbysection[$navsection]))
				$navbysection[$navsection] = array();
			$t = $args[0];
...
function private_addnav($text,$link=false,$priv=false,$pop=false,$popsize="500x300"){
	...
	if ($session['user']['prefs']['nocolornavs']){
		require_once("lib/sanitize.php");
		if (is_array($t)){
			$text[0] = sanitize($text[0]);
		} else {
			$text = sanitize($text);
		}
	}
		...
		if ($text!=""){
				...
				if ($text[1] == $keys[$j]) {
					...
					if ($hchar==' ' || array_key_exists($hchar,$accesskeys) && $accesskeys[$hchar]==1){
						...
						if ($found==false) {
							...
							if ($session['user']['prefs']['disable_hotkeys'] != 1){
			...
			if ($key=="" || $key==" "){
				...
				if ($pop){
					...
					global $SCRIPT_NAME;
					//$quickkeys[$key]="initiateState('$link$extra')";
					if ($SCRIPT_NAME == 'forest.php') $quickkeys[$key]="window.location = '$link$extra'";
