...
//	debuglog("gained $winamount ({$badguy['creaturegold']} base) gold and $wonexp exp (loser lost $lostexp) for killing ", $badguy['acctid']);
	...
	$sqlz = "SELECT prefs FROM accounts WHERE acctid=".(int)$badguy['acctid']."";
	$resz = db_query($sqlz);
	$rowz = db_fetch_assoc($resz);
	$prefs = unserialize($rowz['prefs']);
	
	if ($prefs['pvpnotif']==1 || !is_numeric($prefs['pvpnotif'])){
...
function pvpdefeat($badguy, $killedloc, $taunt, $options)
{
	...
	if ($prefs['pvpnotif']==1 || !is_numeric($prefs['pvpnotif'])){	
		...
		systemmail($badguy['acctid'],array("`2You were successful while you were in %s`2", $killedloc),array($msg,$session['user']['name'], $killedloc, $wonexp,$winamount, $args['pvpmsgadd']));
