...
// Created by Aaron and Stephen.

function soap_email($email){
 	$helo = "helo example.com\r\n";
 	$mailfrom = "mail from: <xythenstaff@gmail.com>\r\n";
 	$banned_emails = array("squidward931@gmail.com","mlr4869@yahoo.com","randallt@atc.edu");
 	if(filter_var($email, FILTER_VALIDATE_EMAIL)){
   		$email_str = explode("@",$email); 
   		if (getmxrr($email_str[1], $mxhosts)){
		    $sock = @fsockopen ( $mxhosts[0], 25, $errno, $errstr, 5);
	     	if (!$sock){
...
function check_proxy(){
	$valid = "VALID";
	$proxy_headers = array('HTTP_VIA','HTTP_X_FORWARDED_FOR','HTTP_FORWARDED_FOR','HTTP_X_FORWARDED','HTTP_FORWARDED','HTTP_CLIENT_IP',
  						'HTTP_FORWARDED_FOR_IP','VIA','X_FORWARDED_FOR','FORWARDED_FOR','X_FORWARDED','FORWARDED','CLIENT_IP','FORWARDED_FOR_IP','HTTP_PROXY_CONNECTION');
  	foreach($proxy_headers as $x){
	    if (isset($_SERVER[$x])) $valid = "INVALID";
	}
	
//	$ports = array(8080,80,81,1080,6588,8000,3128,553,554,4480);
//	foreach($ports as $port){
//		if (fsockopen($_SERVER['REMOTE_ADDR'], $port, $errno, $errstr, 5)) $valid = "INVALID";
//	}
