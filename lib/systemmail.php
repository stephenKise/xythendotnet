...
function systemmail($to,$subject,$body,$from=0,$noemail=false){
	...
	$sql = "INSERT INTO " . db_prefix("mail") . " (msgfrom,msgto,subject,body,sent,seen,originator) VALUES ('".$from."','".(int)$to."','$subject','$body','".date("Y-m-d H:i:s")."', '1', ".($session['user']['acctid']).")";
	db_query($sql);
	
	$check_for_mk = explode('---',$body);
	//debug($check_for_mk[0]);
	// Mail Keeper
	if (db_table_exists(db_prefix('mailkeeper')) && $from <> 0){
		$mailkeeper = "INSERT INTO " . db_prefix("mailkeeper") . " VALUES ('','$from','$to','$subject','".addslashes($check_for_mk[0])."','".date("Y-m-d H:i:s")."','')";
		db_query($mailkeeper);
	}
...
// 		else
// 			$fromline=translate_inline("Realm of Xythen","mail");
