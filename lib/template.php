...
function templatereplace($itemname,$vals=false){
	global $template;
	$ignore_these_tags = array("petitioncount");
	if (!isset($template[$itemname]) && !in_array($itemname, $ignore_these_tags))
	...
	while (list($key,$val)=@each($vals)){
		if (strpos($out,"{".$key."}")===false){
			if (!in_array($itemname,$ignore_these_tags))
			{
				output("`bWarning:`b thex `i%s`i piece was not found in the `i%s`i te".
...
function prepare_template($force=false){
	...
	$templatename="";
	$templatemessage="";
	if (!file_exists("templates/{$_COOKIE['template']}")) $_COOKIE['template'] = "Responsive.htm";
	if ($_COOKIE['template']!="")
		$templatename=$_COOKIE['template'];
	if ($templatename=="" || !file_exists("templates/$templatename"))
		$templatename=getsetting("defaultskin", "Responsive.htm");
	if ($templatename=="" || !file_exists("templates/$templatename"))
		$templatename="Responsive.htm";
