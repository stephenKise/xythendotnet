...
if ($display == 1){
	...
	for ($i=0;$i<$number3;$i++){
		...
		modulehook('search_usereditor',$row);
		...
		rawoutput("<td nowrap>");
		rawoutput("[ <a href='user.php?op=edit&userid={$row['acctid']}$m'>$ed</a> | <a href='user.php?op=del&userid={$row['acctid']}' onClick=\"return confirm('$conf');\">$del</a> | <a href='user.php?op=bans&subop=add_ban&userid={$row['acctid']}'>$ban</a> | <a href='user.php?op=debuglog&userid={$row['acctid']}'>$log</a> ]");
		addnav("","user.php?op=edit&userid={$row['acctid']}$m");
		addnav("","user.php?op=del&userid={$row['acctid']}");
		addnav("","user.php?op=bans&subop=add_ban&userid={$row['acctid']}");
		...
		rawoutput("</td><td>");
		// output_notl("%s <a href='user.php?op=ban_email&email=%s'>`4[`2Ban`4]</a>", $row['emailaddress'],$row['emailaddress'],true);
		output_notl("%s ", $row['emailaddress'],true);
		addnav('',"user.php?op=ban_email&email={$row['emailaddress']}");
		rawoutput("</td></tr>");
