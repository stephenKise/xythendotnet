...
if ($session['user']['superuser'] & SU_EDIT_DONATIONS) addnav("Add donation points","donators.php?op=add1&name=".rawurlencode($row['login'])."&ret=".urlencode($_SERVER['REQUEST_URI']));
addnav("Make Moderator","user.php?op=edit&userid=$userid&subop=mod");
addnav("Make Administrator","user.php?op=edit&userid=$userid&subop=admin");
addnav("Remove as Staff","user.php?op=edit&userid=$userid&subop=remove");
...
}elseif(httpget("subop")=="mod"){
	db_query("UPDATE accounts SET superuser='75551262' WHERE acctid=$userid LIMIT 1");
	output("`c`bYou have made this user a Moderator.`b`c");
	debuglog("has made $userid a moderator");
}elseif(httpget("subop")=="admin"){
	db_query("UPDATE accounts SET superuser='75750366' WHERE acctid=$userid LIMIT 1");
	output("`c`bYou have made this user an Admin.`b`c");
	debuglog("has made $userid an admin");
}elseif(httpget("subop")=="remove"){
	db_query("UPDATE accounts SET superuser='0', petition_sub='0'  WHERE acctid=$userid LIMIT 1");
	output("`c`bYou have remove this user from staff.`b`c");
	debuglog("has removed $userid from staff");
}
