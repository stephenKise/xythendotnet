	...
	if (isset($userinfo[$key])){
		if ($key=="newpassword" ){
			...
			if ((int)$value != (int)$oldvalues['superuser']){
				...
				stafflog("",$session['user']['acctid'],$userid,"".$session['user']['name']." changed superuser to ".show_bitfield($value)."   ","0");
			...
			stafflog("",$session['user']['acctid'],$userid,"" . $session['user']['name'] . " changed player name to $newname","0");
			...
			stafflog("",$session['user']['acctid'],$userid,"".$session['user']['name']." changed $key to $val","0");
			if ($session['user']['acctid']==$userid) {
...
redirect('user.php?op=edit&userid='.httpget('userid'));
?>