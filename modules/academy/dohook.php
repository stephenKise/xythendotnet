	...
	switch ($hookname){
		...
		case "biostat":
			if (get_module_pref("active","academy",$args['acctid'])){
				...
				$args['tablebiostat']['Companions/Items'][$scolor.$classarray[$class].' Name'] = $name;
				$args['tablebiostat']['Companions/Items'][$scolor.$classarray[$class].' Level'] = $level;
				$args['tablebiostat']['Companions/Items'][$scolor.$classarray[$class].' Placement in HOF'] = $yournumber." of ".$num1;
			}
			break;
		case "dragonkill":
