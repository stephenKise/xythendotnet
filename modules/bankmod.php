...
function bankmod_getmoduleinfo(){
	$info = array(
		...
		"prefs"=>array(
			...
			"gemloss"=>"Has user lost gems this game day,bool|0",
		)
	);
	return $info;
}
	...
	switch($hookname){
		...
		/*case "charstats":
			...
			if (get_module_pref("gemsinbank") > 0){ 
				$bankedgems .= " `7(" . get_module_pref("gemsinbank") . " `7Banked)";
			}
			// setcharstat("Personal Info", "Gems", $bankedgems);
			$goldinhand = getcharstat("Personal Info", "Gold");
			$bankedgold = $goldinhand;
			if (($session['user']['goldinbank']) > 0){
				$bankedgold .= " `7(" . $session['user']['goldinbank'] . " `7Banked)";
			// setcharstat("Personal Info", "Gold", $bankedgold);
			}
			break;
			*/
	}
...
function bankmod_run(){
	...
	output("`^`c`b`^B`b`i`ma`Mn`tk`i `)o`7f `b`LS`b`i`ly`kl`3i`#s`3t`ke`ln`i`c");
	$op = httpget('op');
	if($op=="deposit"){
		output("`0");
		rawoutput("<form action='runmodule.php?module=bankmod&op=depositfinish' method='POST'>");
		output("`6Jezabelle says, \"`3You have a total of `^%s`3 gems in the bank.`6\"`n", get_module_pref("gemsinbank"));
	  ...
	  addnav("","runmodule.php?module=bankmod&op=depositfinish");
		...
		$notenough = translate_inline("`\$ERROR: Not enough gems in hand to deposit.`n`n`^You plunk your `&%s`^ gems on the counter and declare that you would like to deposit all `&%s`^ gems of them.`n`n`6Jezabelle stares blandly at you for a few seconds until you become self conscious and recount your gems, realizing your mistake.");
		$depositbalance= translate_inline("`6 Jezabelle records your deposit of `^%s `6gems in her ledger. \"`3Thank you, `&%s`3.  You now have a total of `^%s`3 gems in the bank and `^%s`3 gems in hand.`6\"");
		...
		output("`6Jezabelle scans through her ledger, \"`3You have a total of `^%s`3 gems in the bank.`6\"`n",get_module_pref("gemsinbank"));
		...
		if ($amount>get_module_pref("gemsinbank")) {
			...
			output("Jezabelle looks at you for a few moments without blinking, then advises you to take basic arithmetic.  You realize your folly and think you should try again.");
		}else{
			$gemsinbank = get_module_pref("gemsinbank");
			$gemsinbank-=$amount;
			set_module_pref("gemsinbank",$gemsinbank);
			...
			output("`6Jezabelle records your withdrawal of `^%s `6gems in her ledger. \"`3Thank you, `&%s`3.  You now have a total of `^%s`3 gems in the bank and `^%s`3 gems in hand.`6\"", $amount,$session['user']['name'], abs(get_module_pref("gemsinbank")),$session['user']['gems']);
		}
	}elseif($op=="instantgold"){
		$amount=$session['user']['gold'];
		$session['user']['goldinbank']+=$amount;
		$session['user']['gold']=0;
		output("`6Jezabelle records your deposit of `^%s `6gold in her ledger. \"`3Thank you, `&%s`3.  You now have a total of `^%s`3 gold in the bank and `^0`3 gold in hand.`6\"",$amount,$session['user']['name'], abs($session['user']['goldinbank']));
		...
		set_module_pref("gemsinbank",$gemsinbank);
		$session['user']['gems']=0;
		output("`6El records your deposit of `^%s `6gems in her ledger. \"`3Thank you, `&%s`3.  You now have a total of `^%s`3 gems in the bank and `^0`3 gems in hand.`6\"",$amount,$session['user']['name'], abs(get_module_pref("gemsinbank")));
		/* if ($gemsinbank>get_module_setting("maxgems")){
			output("`n`nElessa glances at her ledger again before closing it and seems to notice something, looking up she says \"`3You know that our bank can only store `^%s`3 gems for you safely, we are not responsible for any gems that are lost or damaged if you deposit too many.\"",get_module_setting("maxgems"));
		} */
	}elseif($op=="instant"){
		...
		set_module_pref("gemsinbank",$gemsinbank);
		$session['user']['gems']=0;
		output("`6Jezabelle records your deposit of `^%s `6gold and `^%s `6gems in her ledger. \"`3Thank you, `&%s`3.  You now have a total of `^%s`3 gold and `^%s`3gems in the bank.`6\"",$amount,$amount2,$session['user']['name'], abs($session['user']['goldinbank']), abs(get_module_pref("gemsinbank")));
		/* if ($gemsinbank>get_module_setting("maxgems")){
			output("`n`nElessa glances at her ledger again before closing it and seems to notice something, looking up she says \"`3You know that our bank can only store `^%s`3 gems for you safely, we are not responsible for any gems that are lost or damaged if you deposit too many.\"",get_module_setting("maxgems"));
		} */
	}
