...
function buyablog_getmoduleinfo(){
	$info = array(
		...
		"version"=>"1.0",
		"category"=>"Lodge",
		"download"=>"http://dragonprime.net/dls/buyablog.zip",
		"settings"=>array(
	...
	switch ($op) {
		...
		case "confirm":
			...
			if($pointsavailable >= $cost){
				...
				output("`n`7\"`&You can also set up a signature. If you don't want one, you can leave now.`7\" reminds he.");
				addnav("More");
		...
		case "signature":
			...
			rawoutput("<input type='input' name='signature' value='".addslashes(get_module_pref("blogsig","mightyblogs"))."'><br><br>");
