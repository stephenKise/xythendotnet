...
function cedrikspotions_getmoduleinfo(){
	$info = array(
		...
		"settings"=>array(
			...
			"charmcost"=>"Cost for Charm Potion,range,10,100,1|2",
			"maxcost"=>"Cost for Vitality Potion,range,10,100,1|2",
			"tempcost"=>"Cost for Health Potion,range,10,100,1|2",
			"forgcost"=>"Cost for Forgetfulness Potion,range,10,100,1|2",
			"transcost"=>"Cost for Transmutation Potion,range,10,100,1|2",
...
function cedrikspotions_run(){
	...
	if($op=="gems"){
		if ($gemcount==""){
			...
			output("`n`u`c`b`\$The max amount of HP you can have is: `&$hpmath`\$!`u`b`c");
		}else{
