...
function cityprefs_run() {
	...
	$cityname=get_cityprefs_cityname("cityid",$cityid);
		...
		addnav("Navigation");
		if(is_module_active("cities"))	addnav(array("%s",$cityname),"runmodule.php?module=cities&op=travel&city=".urlencode($cityname)."&su=1");
		else addnav(array("%s",$cityname),"village.php");
	}
	addnav("Navigation");	
   ...
   if($op!="su") addnav("Back to city list","runmodule.php?module=cityprefs&op=su");
    switch ($op) {
        ...
        case "editmodulesave":
			addnav("Operations");
			addnav("Edit Name","runmodule.php?module=cityprefs&op=editcity&cityid=$cityid");
			addnav("Delete","runmodule.php?module=cityprefs&op=delcity&cityid=$cityid");
			$mdule = httpget("mdule");
			if($mdule==""){
