...
function clanbuffs_getmoduleinfo(){
        $info = array(
				...
				"regeninc"=>"Cost to upgrade regen (increment), int|50",
				...
				"eroundbase"=>"Rounds (base), range,0,50,1|0",
				"eroundinc"=>"Rounds (increment), range,0,50,1|10",
                        "Max Levels,title",
				"maxatk"=>"Maximum Attack Level, range,0,100,1|10",
				"maxdef"=>"Maximum Defense Level, range,0,100,1|10",
				"maxdrain"=>"Maximum Drain HP Level, range,0,100,1|10",
				"maxthorn"=>"Maximum Thorn Level, range,0,100,1|10",
				"maxregen"=>"Maximum Regen Level, range,0,100,1|10",
				"maxround"=>"Maximum Round Level, range,0,100,1|15",
                ),
                "prefs"=>array(
                ...
                "prefs-clans"=>array(
                  ...
                  "gold"=>"gold stored in buff bank,int|0",
...
function clanbuffs_dohook($hookname,$args){
		...
		if (get_module_setting("hof")) {
             ...
             addnav("Guild Buffs","runmodule.php?module=clanbuffs&op=hof");
		}
	  break;
        case "footer-clan":
             if($session['user']['clanid']!=0 and httpget("op")==""){
                if($session['user']['clanrank']>0){
                   addnav("Management");
                   addnav("Guild Buffs","runmodule.php?module=clanbuffs&op=enter");
                }
             }
        break;
        case "village":
		...
		if (get_module_pref("refreshbuff")) {
		   ...
		   apply_clan_buff_for_one();
                   output("`n`n`c`b`!Your guild's aura has been upgraded and refreshed!`0`b`c`n`n");
		}
        break;
        case "newday":
              if ($session['user']['clanid'] != 0 && $session['user']['clanrank'] > 0) {
                if(get_module_objpref("clans", $session['user']['clanid'],"buffactive")==1){
                   include_once("modules/clanbuffs/clanbuffs_func.php");
                   apply_clan_buff_for_one();
                   //output("`n`^Your guild's aura protects you!`n");
                }
             }
        break;
        case "changesetting":
