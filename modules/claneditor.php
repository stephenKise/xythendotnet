 ...
 * Guilds can be created, deleted, and modified
 * You can set Guild Prefrences as you can with mounts and drinks
Version Log:
 v1.1:
...
function claneditor_dohook($hookname,$args) {
	...
	case "superuser":
		if ($session['user']['superuser'] & SU_EDIT_USERS) {
			addnav("Editors");
			addnav("Guild Editor","runmodule.php?module=claneditor&admin=true");
		}
		break;
	}
	return $args;
...
function claneditor_run(){
	...
	addnav("List Guilds","runmodule.php?module=claneditor&op=list");
	addnav("Create a New Guild","runmodule.php?module=claneditor&op=new");
	if ($op==""||$op=="list"){
		...
		$result = db_query($sql);
		if (db_num_rows($result)>0){
			output("`%Karissa`7 steps in as if by magic, listing the Guilds for you.`n");
			output("`@`bList of Guild:`b`0`n`n");
			$v = 0;
			$memb_n = translate_inline("(%s members)");
			$memb_1 = translate_inline("(%s member)");
			rawoutput('<table cellspacing="0" cellpadding="2" align="left">');
			output_notl("<tr class='trhead'><td>%s</td><td>%s</td></tr>",translate_inline("`b`&Name of Guild`b"),translate_inline("`&`iNumber of Members`i"),true);
			...
			output("`7There are no Guilds in the database.`n`c");
		}
		rawoutput("</td></tr><tr><td>");
		output_notl("<br>[<a href='runmodule.php?module=claneditor&op=new'>%s</a>]",translate_inline("Create a New Guild"),true);
		...
		if ($apply==1){
			...
			$result = db_query($sql);
			$e = translate_inline("`%Karissa`7 hands you a form to start a Guild.");
			$e1 = translate_inline("`%Karissa`7 looks over your form but informs you that your Guild name must consist only of letters, spaces, apostrophes, or dashes.  Also, your short name can consist only of letters. She hands you a blank form.");
			$e2 = translate_inline("`%Karissa`7 looks over your form but informs you that you must have at least 5 and no more than 50 characters in your Guild's name (and they must consist only of letters, spaces, apostrophes, or dashes), then hands you a blank form.");
			$e3 = translate_inline("`%Karissa`7 looks over your form but informs you that you must have at least 2 and no more than 5 characters in your Guild's short name (and they must all be letters), then hands you a blank form.");
			$e4 = translate_inline("`%Karissa`7 looks over your form but informs you that the Guild name %s is already taken, and hands you a blank form.");
			...
			if ($ocs==""&&$ocn==""&&!httppostisset('clanname')&&!httppostisset('clanshort')) {
				...
				if (db_num_rows($result)>0){
					...
					db_query($sql);
					$subj = "New Guild!";
					$msg = array("%s`0`^ has made you a new Guild!",$session['user']['name']);
					...
					output("Congratulations, you've created a new Guild named %s!",stripslashes($clanname));
				}
			}
		}elseif($apply==0){
			clanuserform();
		}else{
			output("`7You teleport to the Guild Hall...");
			output("`7You approach `%Karissa`7 and mention that you would like to start a new Guild.");
			output("She tells you that there are three requirements to starting a Guild.");
			output("First, you have to decide on a full name for your Guild.");
			output("Second, you have to decide on an abbreviation for your Guild.");
			output("Third you have to decide on the person that should run the Guild.");
		...
		if ($op!="deleteclan") {
			page_header("The Guild of %s", full_sanitize($claninfo['clanname']));
			output("`n`c`^`bThe Guild of %s`b`c`n`n", full_sanitize($claninfo['clanname']));
		} else {
			page_header("Guildal Deletion");
		}
		if ($op=="mview"){
			...
			commentdisplay("", "clan-{$claninfo['clanid']}","Speak into their Guild",25,"projects");
			output_notl("`n`n");
			...
			if (httpget("sop")=="yes") {
				...
				$msg = array("The Guild you were in, %s, has closed its doors.\nSorry for any inconvenience.",$claninfo['clanname']);
				while ($row = db_fetch_assoc($result)){
				...
				unset($claninfo);
				$dt = "";
				output("That Guild has been wiped.`n");
				output("`@Users within the Guild have been notified.");
			} else {
				output("`%`c`bAre you SURE you want to delete this Guild?`b`c`n");
				$dc = translate_inline("Delete this Guild? Are you sure!");
			...
			page_header("Update Guild Information");
			...
			if (httppostisset('clanname') && $clanname!=$claninfo['clanname']){
				...
				output("Updating the Guild name`n");
				db_query($sql);
			...
			$blocked = translate_inline("The Guild has been blocked from posting a description.`n");
			...
			output("This is the Guild current membership:`n");
			...
			$remove = translate_inline("Remove From The Guild");
			$confirm = translate_inline("Are you sure you wish to remove this member from the Guild?");
		...
		if ($dt!=""&&isset($claninfo)) {
			...
			addnav("Update Guild Information","runmodule.php?module=claneditor&op=updinfo&dt=$dt");
			addnav("Delete this Guild","runmodule.php?module=claneditor&op=deleteclan&dt=$dt");
...
function clanuserform(){
	...
	output("`^`b`cNew Guild Form`c`b");
	output("`nFirst things first, choose who should run the Guild.");
	output("Name of user (cannot be in a Guild): ");
