<?php
			if (get_module_setting("dktax")==1) {
				...
				if (get_module_setting("allowgoldinvault")==1) {
					...
					$msg = sprintf("::`7killed `@a Crystal Guardian`7 and the Guild has been taxed `$%s `^gold.", ($gold-$goldaftertax));
				}
				if (get_module_setting("allowgemsinvault")==1) {
					...
					$msg = sprintf("::`7killed `@a Crystal Guardian`7 and the Guild has been taxed `$%s `@gems.", ($gems-$gemsaftertax));
				}	
				if (get_module_setting("allowgemsinvault")==1 and get_module_setting("allowgoldinvault")==1) {
					$msg = sprintf("::`7killed `@a Crystal Guardian`7 and the Guild has been taxed `$%s `^gold`7 and `$%s `@gems.", ($gold-$goldaftertax), ($gems-$gemsaftertax));
				}
				$name = $session['user']['name'];
				if ($gold>0) {
					if (get_module_setting("taxannouncement")==1) injectcommentary("clan-".$session['user']['clanid'], "", $msg, $schema=false);
					output("`nYour Guild has to pay taxes because of the most recent Crystal Guardian kill.");
					...
					while ($row = db_fetch_assoc($result)) {
						if (get_module_pref('check_showNot','clanvault',$row['acctid'])==1) {
							...
							$msg = array("A member of your Guild, %s, `^has slain a Crystal Guardian which means that your Guild has to pay a special Tentromech kill tax.`nConsidering the amount of members in your Guild the tax is %s gold and %s gems.`n`nSincerely,`nTaxman", $session['user']['name'], ($gold-$goldaftertax), ($gems-$gemsaftertax));
