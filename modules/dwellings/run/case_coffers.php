...
//Added the commentary/log thingy at the bottom of the page.
	...
	$sql2 = db_query("SELECT dwgold,dwgems FROM accounts WHERE acctid='".$row['ownerid']."'");
	$row2 = db_fetch_assoc($sql2);
	$coffersgold = $row2['dwgold'];
	$coffersgems = $row2['dwgems'];
	...
	switch ($subop){
		case "with":
			if ($cofferwiths > 0){
				...
				if ($amount > $cofg){
					...
					db_query("UPDATE accounts SET $dw=$dw-$amount WHERE acctid=$ownerid");
		...
		case "dep":
			if ($cofferdeps > 0){
				...
				if ($amount > 0){
					...
					if ($g == "gems") $dw = "dwgems";
					$cofg = $row2[$dw];
					$sql2 = "SELECT SUM($dw) AS $dw FROM acctid  WHERE acctid=$ownerid";
		...
		if($subop != ""){
			$sql = "SELECT ownerid,gold,gems,type FROM " . db_prefix("dwellings") . " WHERE dwid='$dwid'";
			$result = db_query($sql);
			$row = db_fetch_assoc($result);
			$type = $row['type'];
			output("`n`n");
		}
		output("`c`2You walk up to your coffers, open them and try to decide what you want to do with your gold and gems.`c`n`n");
