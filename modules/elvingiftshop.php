<?php
function elvingiftshop_getmoduleinfo(){	$info = array(
...
function elvingiftshop_install(){	module_addhook("village");	return true;
}

function elvingiftshop_uninstall(){	return true;
}

function elvingiftshop_dohook($hookname,$args){	global $session;	switch($hookname){		case "village":
			if ($session['user']['location'] == get_module_setting("gsloc")){
				tlschema($args['schemas']['marketnav']);    
				addnav($args['marketnav']);    
				tlschema();
				addnav(array("i?`b`mP`b`Mh`b`mi`b`yl`i`ga`i`kn`b`Kt`b`kh`i`gr`yo`i`mp`b`Ei`b`Me`ms ",get_module_setting('gsowner')), "runmodule.php?module=elvingiftshop");
			}
		break;	}	return $args;
}
...
function elvingiftshop_run(){
	...
	if ($op==""){
		...
		for ($i=1;$i<13;$i+=1){			$currentgift = "gift".$i;			$currentprice = "gift".$i."price";			if ($session['user']['gold'] >= get_module_setting($currentprice)){				output("%s `&%s`0 `3- %s gold%s","<a href=\"runmodule.php?module=elvingiftshop&op=send&gid=".$i."\">",get_module_setting($currentgift),get_module_setting($currentprice),"</a><br>",TRUE);
				addnav("","runmodule.php?module=elvingiftshop&op=send&gid=".$i."");			}
		}
		addnav("Back to Sylisten","village.php");
	}
	
	if ($op == "send"){		output("To whom would you like to send your gift to?`n");		rawoutput("<form action='runmodule.php?module=elvingiftshop&op=send2&gid=$gift' method='POST'>");		rawoutput("<p><input type=\"text\" name=\"whom\" size=\"37\"></p>");		rawoutput("<p><input type=\"submit\" value=\"Submit\" name=\"B1\"><input type=\"reset\" value=\"Reset\" name=\"B2\"></p>");		rawoutput("</form>");		addnav("","runmodule.php?module=elvingiftshop&op=send2&gid=$gift");		addnav("Go Back","runmodule.php?module=elvingiftshop");
	}
 	...
 	if ($op == "send2"){		$whom = addslashes("%".implode("%",str_split(httppost('whom')))."%");		output("Choose who to send your gift to:`n");		rawoutput("<table cellpadding='3' cellspacing='0' border='0'>");		rawoutput("<tr class='trhead'><td>Name</td><td>Level</td></tr>");		$i = 0;		$sql = "SELECT login,name,level,acctid FROM ".db_prefix("accounts")." WHERE name LIKE '%".$whom."%' and acctid <> '".$session['user']['acctid']."' ORDER BY level,login LIMIT 100";		$result = db_query($sql);		while ($row = db_fetch_assoc($result)){
			rawoutput("<tr class='".($i%2?"trlight":"trdark")."'><td>");
				addnav("","runmodule.php?module=elvingiftshop&op=send3&gid=$gift&name=".HTMLEntities($row['acctid']));				rawoutput("<a href='runmodule.php?module=elvingiftshop&op=send3&gid=$gift&name=".HTMLEntities($row['acctid'])."'>");				output_notl($row['name']);				rawoutput("</a>");
			rawoutput("</td><td>");
				output_notl($row['level']);
			rawoutput("</td></tr>");			$i++;
		}		if (!$i){			rawoutput("<tr class='trlight'><td colspan='2'>");			output("No-One Found!");			rawoutput("</td></tr>");		}
		rawoutput("</table>");
		output("`n");		addnav("Go Back","runmodule.php?module=elvingiftshop");	}
	
	if ($op=="send3"){		output("Fill in the Note Card that goes with the gift.`n");		output("Leave blank for no note.");		rawoutput("<form action='runmodule.php?module=elvingiftshop&op=send4&gid=$gift&name=$name' method='POST'>");		rawoutput("<p><input type=\"text\" name=\"mess\" size=\"37\"></p>");		rawoutput("<p><input type=\"submit\" value=\"Submit\" name=\"B1\"><input type=\"reset\" value=\"Reset\" name=\"B2\"></p>");		rawoutput("</form>");		addnav("","runmodule.php?module=elvingiftshop&op=send4&gid=$gift&name=$name");		addnav("Go Back","runmodule.php?module=elvingiftshop");
	}
	...
	if ($op=="send4"){		$mess = httppost('mess');		$price = (int)get_module_setting("gift".$gift."price");		$thegift = get_module_setting("gift".$gift."");		$session['user']['gold']-=$price;		$mailmessage=$session['user']['name'];		$mailmessage.="`7 has sent you a gift.  When you open it you see it is a `6";		$mailmessage.=$thegift;		$mailmessage.="`7 from ".get_module_setting('gsowner')."'s Philantrophie Shop!`n`n";		if ($mess <> ""){
			$mailmessage.="The attached note says \"";			$mailmessage.=$mess;			$mailmessage.=".\"";		}		systemmail($name,"`2You have recieved a gift!`2",$mailmessage);		output("You gift of a %s has been sent!",$thegift);		addnav("Continue","runmodule.php?module=elvingiftshop");
	}
