...
function mutemod_getmoduleinfo(){
	$info = array(
		...
		"version"=>"2.0",
		"author"=>"Sneakabout`7, modified by `i`b`&Xpert`b`i",
		...
		"permoocbanned"=>"Is this player Permanently OOC Chat banned?,bool|0",
		"tempoocbanned"=>"Is this player Temporarily OOC Chat banned?,int|0",
		)
	);
	return $info;
}
		...
		case "bioinfo":
			...
			if ($session['user']['superuser'] & SU_EDIT_COMMENTS) {
				...
				if ($muted) {
					...
					addnav("U?Un-mute player", "runmodule.php?module=newbio&char=".$id."&ret=".rawurlencode(httpget("ret"))."&op=unmute");
				} else {
					addnav("&?Mute player", "runmodule.php?module=newbio&char=".$id."&ret=".rawurlencode(httpget("ret"))."&op=mute");
				}
				if ($tmuted) {
					if ($tmuted == 1){
					...
					addnav("X?Extend tempmute", "runmodule.php?module=newbio&char=".$id."&ret=".rawurlencode(httpget("ret"))."&op=exttempmute");
					addnav("U?Un-tempmute player", "runmodule.php?module=newbio&char=".$id."&ret=".rawurlencode(httpget("ret"))."&op=untempmute");
				} else {
					addnav("*?Tempmute player", "runmodule.php?module=newbio&char=".$id."&ret=".rawurlencode(httpget("ret"))."&op=tempmute");
				}
			}
		break;
		case "newday":
...
function mutemod_domute($id){
	...
	if ($op=="mute") {
		modulehook("mute", array("when"=>date("Y-m-d H:i:s"), "userid"=>$id, "staffid"=>$session['user']['acctid']));
		set_module_pref("muted",1,false,$id);
		output("`nYou have perm-muted this player. This will last until it is lifted, by you or another member of staff!`0`n");
	} elseif ($op=="tempmute") {
		modulehook("tempmute", array("when"=>date("Y-m-d H:i:s"), "userid"=>$id, "staffid"=>$session['user']['acctid']));
		set_module_pref("tempmute",1,false,$id);
		output("`nTempmutes will be lifted after one gameday. You can extend it if you wish.`0`n");
	} elseif ($op=="exttempmute") {
		$tmuted = get_module_pref("tempmute",false,$id)+1;
		set_module_pref("tempmute",$tmuted,false,$id);
		output("`nThis player can't talk until %s days have passed!`0`n", $tmuted);
	}
}
?>