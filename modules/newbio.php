...
// 		}else{
		...
		output_notl("<big>`c$scolor Biography for %s`^.",$target['name'],TRUE);
		...
		output($scolor);
		$target['tablebiostat']['Basic Info'][$scolor.'Title'] = $target['title'];
		$target['tablebiostat']['Basic Info'][$scolor.'Level'] = $target['level'];
		$loggedin = false;
		...
		$target['tablebiostat']['Basic Info'][$scolor.'Status'] = $status;
		$target['tablebiostat']['Basic Info'][$scolor.'Resurrections'] = $target['resurrections'];
		//$race = $target['race'];
		//if (!$race) $race = RACE_UNKNOWN;
		//tlschema("race");
		//$race = translate_inline($race);
		tlschema();
		//$target['tablebiostat']['Basic Info'][$scolor.'Race'] = $race;
		$genders = translate_inline(array("Male","Female"));
		$target['tablebiostat']['Basic Info'][$scolor.'Gender'] = $genders[$target['sex']];
		...
		if ($target['weapon']>0) $target['tablebiostat']['Items Info'][$scolor.'Weapon'] = $target['Weapon'];
		if ($target['armor']>0) $target['tablebiostat']['Items Info'][$scolor.'Armor'] = $target['Armor'];
		if ($target['dragonkills']>0) $target['tablebiostat']['Basic Info'][$scolor.'TentroKills'] = $target['dragonkills'];
		$arr = array(0 => "`@Common",500 => "`eBronze",1000 => "`7Silver",5000 => "`^Gold",10000 => "`jPlatinum",20000 => "`&Diamond");
		foreach($arr as $key => $val){
			if ($target['donation'] >= $key) $out = $val;
		}
		$target['tablebiostat']['Basic Info'][$scolor.'Donation Grade'] = $out;
		$target['tablebiostat']['Basic Info'][$scolor.'Charm'] = $target['charm'];
		$target['tablebiostat']['Companions/Items'][$scolor.'Mount Name'] = $mount['mountname'];
		if ($target['bio'] > "") $target['tablebiostat']['Basic Info'][$scolor.'Bio'] = soap($target['bio']);
