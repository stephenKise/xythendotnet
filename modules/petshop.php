...
function petshop_dohook($hookname,$args){
	...
	if (($haspet == 1) && ($session['user']['alive'] == true)){
		if ($args['type'] == "dragon") {
			output("%s `2seems visibly nervous at the sight of the `@Tentromech!`n", $customname);
    ...
    $pettype = translate_inline("`@$petname `@the $petbreed");
    $petnone = translate_inline("`@None");
	if ($ownspet == 1){
		$args['tablebiostat']['Companions/Items'][$scolor.'Pet'] = $pettype;
	}else{
	...
	if ($haspet==1){
		...
		$moodnum = e_rand(1,7);
		$moodlist = array(1=>"Happy",2=>"Curious",3=>"Content",4=>"Hyper",5=>"Bored",6=>"Sleepy",7=>"Wants to Potty",8=>"Derpy");
		$mood = translate_inline($moodlist[$moodnum]);
		$turns = get_module_pref("petturns");
		addcharstat("Pet Info");
		addcharstat("Name", $customname);
		if ($session['user']['alive'] == false){
			addcharstat("Mood", $moodgone);
		}else if ($neglect == 1){
			addcharstat("Mood", $moodmad);
		}else{
			addcharstat("Mood", $mood);
		}
		if(get_module_pref("petattack")) addcharstat("Turns",$turns);
	}
	break;
	case "dragonkill":	
