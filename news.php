...
$sql2 = "SELECT " . db_prefix("motd") . ".*,name AS motdauthorname FROM " . db_prefix("motd") . " LEFT JOIN " . db_prefix("accounts") . " ON " . db_prefix("accounts") . ".acctid = " . db_prefix("motd") . ".motdauthor WHERE motdtype = 0 ORDER BY motddate DESC LIMIT 1";
...
while ($row = db_fetch_assoc($result)) {
	output("`c`2<hr width='250' color='#635F40'>`c",TRUE);
	if ($session['user']['superuser'] & SU_EDIT_COMMENTS){
		$del = "Del";
		output_notl("`&[ <a href='superuser.php?op=newsdelete&newsid=".$row['newsid']."&return=".URLEncode($_SERVER['REQUEST_URI'])."'>$del</a> `&]&nbsp;",TRUE);
