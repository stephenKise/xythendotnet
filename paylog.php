...
if ($op==""){
	...
	$sql = "SELECT " . db_prefix("paylog") . ".*," . db_prefix("accounts") . ".login," . db_prefix("accounts") . ".name," . db_prefix("accounts") . ".donation," . db_prefix("accounts") . ".donationspent FROM " . db_prefix("paylog") . " LEFT JOIN " . db_prefix("accounts") . " ON " . db_prefix("paylog") . ".acctid = " . db_prefix("accounts") . ".acctid WHERE processdate>='$startdate' AND processdate < '$enddate' ORDER BY payid DESC";
	$result = db_query($sql);
	...
	//rawoutput("<tr class='trhead'><td>Date</td><td>$id</td><td>$gross</td><td>$fee</td><td>$net</td><td>$processed</td><td>$who</td></tr>");
	rawoutput("<tr class='trhead'><td>Date</td><td>$gross</td><td>$fee</td><td>$net</td><td>$processed</td><td>$who</td></tr>");
	...
	for ($i=0;$i<$number;$i++){
		...
		if ($row['name']>"") {
			...
			output_notl("`7[<a href=\"mail.php?op=write&to=".rawurlencode($row['login'])."\" target=\"_blank\" onClick=\"".popup("mail.php?op=write&to=".rawurlencode($row['login'])."").";return false;\">M</a>`7] <a href='user.php?op=edit&userid={$row['acctid']}'>".$row['name']."`0</a>",true);
