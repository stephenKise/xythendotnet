...
function writelog($response){
	...
	if ($match[1]>""){
		...
		if ($acctid>0){
			...
			db_query("UPDATE " . db_prefix("accounts") . " SET donation = donation + '{$hookresult['points']}' WHERE acctid=$acctid");
			$sql = db_query("SELECT name FROM " . db_prefix('accounts') . " WHERE acctid = '$acctid'");
			$row = db_fetch_assoc($sql);
			
			require_once("lib/systemmail.php");
			//systemmail(1,"`b`QPlayer Donation`b","".$name."(ACCTID: ".$acctid.") has donated $".$payment_amount.".`n`nPlease give ".$name." ".($payment_amount*5000)." gold, ".($payment_amount*2000)." gems, and ".($payment_amount*1)." tentrokills. [<a href='user.php?op=edit&userid=$acctid' target='_blank'  onClick=\"".popup("user.php?op=edit&userid=$acctid").";return false;\">Click Here</a>]");
			systemmail(1,"`b`QPlayer Donation`b","{$row['name']} (ACCTID: $acctid) has donated \$$payment_amount.`n`n");
