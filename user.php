...
addnav("Bans");
$id_to_ban = "";
if (httpget('userid') != "") $id_to_ban = "&userid=".httpget('userid')."";
addnav("Add a ban","user.php?op=bans&subop=add_ban".$id_to_ban."");
addnav("List/Remove bans","user.php?op=bans&subop=remove_ban");
...
$sql = "SELECT clanid FROM " . db_prefix("accounts") . " WHERE acctid='$userid'";
...
$userinfo = array(
	...
	"superuser"=>"Superuser Permissions".
		...
		SU_EDIT_EQUIPMENT.  ",Edit Equipment,".
	...
	"transfers"=>"Transfers to this user:,viewonly",
...
{
	array_merge($userinfo, array("ctitle"=>"Custom Title (prepended to name if set)"));
}
...
switch ($op) {
	...
	case "debuglog":
		...
		break;
	case "bans":
		require("lib/user/user_bans.php");
		break;
	case "ban_email":
	$filer = "./game_cache/banned_emails.csv";
		require("lib/user/user_ban_email.php");
		break;
	case "":
